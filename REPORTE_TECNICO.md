...
-   Yeremy Vega Pineda - 20242020233
...
-   Se guarda cada mensaje como un diccionario `{usuario, mensaje}` dentro de una lista protegida por un `Lock` para evitar condiciones de carrera.
...
3. **¿Por qué es importante utilizar estructuras organizadas como JSON?**  
   Porque el cliente y el servidor necesitan por asi decirlo un idioma claro. JSON es simple, legible y se convierte fácil a diccionarios en Python, lo que facilita validar y depurar.
...
6. **¿Qué elementos serían necesarios para extender el servicio hacia almacenamiento persistente?**  
   Necesitaríamos una base de datos o al menos escribir en archivos. Además habría que definir cómo serializar cada mensaje y probablemente agregar índices o alguna especie de filtros para no leer todo en memoria cada vez.
